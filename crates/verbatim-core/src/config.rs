...
mod config_store;
#[path = "config_template.rs"]
mod config_template;
use crate::types::{EdgeType, EmbeddingProfileId, OcrProfile, VectorIndexResidency};
pub use config_store::StoreConfig;
use config_template::DEFAULT_CONFIG_TEMPLATE;
...
mod tests {
    ...
    fn parse_default_template() {
        ...
        assert_eq!(config.store.durability, SqliteDurabilityProfile::Balanced);
...
"#,
    ...
    fn reload_plan_classifies_restart_required_changes() {
        ...
        let mut candidate = current.clone();
        candidate.daemon.bind = "127.0.0.1:9900".into();
        candidate.store.durability = SqliteDurabilityProfile::Durable;
        ...
        assert!(plan.restart_required_keys[3].reason.contains("SQLite"));
        assert!(plan.restart_required_keys[4]
            .reason
            .contains("resident HNSW"));

        let applied = current.apply_reload_safe_changes(&candidate);
        assert_eq!(applied.daemon.bind, current.daemon.bind);
        assert_eq!(applied.store.durability, current.store.durability);
