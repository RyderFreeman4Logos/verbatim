...
fn vector_space_generation_shard_and_manifest_are_bounded_and_round_trip() {
    ...
    let manifest = SsdShardManifest::new(SsdShardManifestFields {
        ...
        checksum: ShardChecksum::new("sha256:0123456789abcdef").expect("checksum"),
    })
...
#[test]
fn all_retrieval_stages_have_hard_output_caps_and_fusion_truncates_before_hydration() {
    assert_eq!(RetrievalStage::ALL.len(), 6);
    let stage_budget = RetrievalStageBudget::new([2, 2, 2, 2, 2, 2]).expect("stage caps");
    ...
    let fused = BoundedCandidates::truncate_fusion(candidates, generation, &stage_budget)
        .expect("fusion truncates before hydration");
    assert_eq!(fused.stage(), RetrievalStage::Fusion);
    assert_eq!(fused.candidates().len(), 2);
    fused
        .advance_to(RetrievalStage::Hydration, &stage_budget)
        .expect("bounded fusion can be hydrated");
}
...
fn strict_small_filtered_candidate_sets_select_exact_simd_scan() {
    ...
    assert_eq!(
        threshold.choose_path(16),
        CandidateGenerationPath::ExactSimdScan
    );
    assert_eq!(
        threshold.choose_path(17),
...
impl VectorSearchContract for ContractStub {
    ...
    fn rescore(
        ...
        candidates.advance_to(RetrievalStage::FullPrecisionRescore, budget.stage_budget())
    }
    ...
    fn hydrate(
        ...
        candidates.advance_to(RetrievalStage::Hydration, budget.stage_budget())
    }
}
...
fn budget_search_candidates_and_serialization_round_trip_through_contract() {
    ...
    let rescored = contract
        ...
        .expect("rescore");
    let hydrated = contract.hydrate(rescored, &search_budget).expect("hydrate");
