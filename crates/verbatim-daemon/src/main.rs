...
fn passage_group_response(input: PassageGroupResponseInput<'_>) -> Result<RetrieveResultResponse> {
    ...
    Ok(RetrieveResultResponse {
        ...
        evidence_id: group.result.provenance.result_id(&first.id).0,
    ...
    display_pack
        ...
        .map(|(index, entry)| {
            ...
            Ok(RetrieveResultResponse {
                ...
                evidence_id: entry.provenance.result_id(&evidence.id).0,
...
mod tests {
    ...
    #[path = "ask_context_only_tests.rs"]
    mod ask_context_only_tests;
