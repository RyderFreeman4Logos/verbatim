...
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
    ...
    test_host_tokuin_independence() {
        ...
        grep -Fq 'monolith-check-tests: PASS (53 Tier-4 cases)' <<<"$output" \
        ...
        printf 'HOST-TOKUIN-INDEPENDENCE: monolith=53 version=12\n'
    }
...
case "$*" in
    ...
    monolith-check-test) printf 'monolith-check-tests: PASS (53 Tier-4 cases)\n' ;;
